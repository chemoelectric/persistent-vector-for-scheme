...
(define (copy-from-pvec! v i w j n)
  ...
  (let* ((i-first i)
    ...
    (if (fx=? i-node-first i-node-last)
      (let-values (((leaf _) (find-entry v i-node-first)))
        (copy-from-leaf! w j leaf k-first (fx+ k-last 1)))
      (let-values (((leaf _) (find-entry v i-node-first)))
        (copy-from-leaf! w j leaf k-first (node-size))
        (let loop ((i-node (fx+ i-node-first (node-size)))
                   (j (fx+ j (fx- (node-size) k-first))))
          (let-values (((leaf _) (find-entry v i-node)))
